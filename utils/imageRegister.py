...
from embedding_utils import get_embedding_model, get_text_embedding
...
def search_registered_api(character, action, threshold, startIndex, limit, dbName, sourceIds=None, image_sim_threshold=0.3, character_weight = 0.45):
    ...
    if query_idx is not None and modified_query:
        query_embedding = get_text_embedding(modified_query)
...
from embedding_utils import get_image_embedding_batch, get_embedding_model
...
def register_images(data_list):
    ...
    for item in processed_items:
        ...
        img_paths = [
        os.path.join(config.WORKING_DIR, p)
       ...
       # print("Embedding shape:", new_emb_np.shape)
        ...
        if metadata_character:
            ...
            combined_vec = np.mean(new_emb_np, axis=0, keepdims=True)
            faiss.normalize_L2(combined_vec)

            #  Add to FAISS
            index.add_with_ids(
                combined_vec,
                np.array([int(new_id)], dtype='int64')
            )

            # Save metadata
            db_manager.add_image_register_metadata(
                name,
                item['new_paths'],
                new_id,
                len(item['new_paths'])
